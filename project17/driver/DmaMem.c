...
static avl_node_t* avltree_remove(avl_node_t* tree, avl_node_t** found_node, DmaMemKey_t key) {
    *found_node = NULL;
    ...
    if ((key.key[0] == tree->key.key[0]) && (key.key[1] == tree->key.key[1])) {
        *found_node = tree;
        tree = do_unlink(tree);
    } else if ((key.key[0] > tree->key.key[0]) || ((key.key[0] == tree->key.key[0]) && (key.key[1] > tree->key.key[1]))) {
...
//    page->alloc_pages  = 0;
//    page->first_pageno = -1;
    set_pages_free(mm, pageno, free_page_size);
